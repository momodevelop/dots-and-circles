                   ...
                   u32 MemorySize) 
{
    ...
    auto* RiffChunk = Stream_ConsumeStruct(wav_riff_chunk, &Stream);
    if (!RiffChunk) {
        return False;
    }
    ...
    auto* FmtChunk = Stream_ConsumeStruct(wav_fmt_chunk, &Stream);
    if (!FmtChunk) {
        return False;
    }
    ...
    auto* DataChunk = Stream_ConsumeStruct(wav_data_chunk, &Stream);
    if (!DataChunk) {
        return False;
    }
